...
#ifndef WIN32
   if (sock == -1) return IFALSE;
#else
   if (sock == INVALID_SOCKET) return IFALSE;
#endif
...
#else
         return (mkdir((((char *)a)+W)) == 0) ? ITRUE : IFALSE;    
#endif 
      case 25: {
         int whence = fixval(c);
...
#else
         return (SetEnvironmentVariable(name + W, val + W) ? IFALSE : ITRUE); }    
#endif         
      default:
         return IFALSE;
   }
